...
export default function Onboarding() {
  ...
  const resolvedCurrency = (customCurrency || currency).toUpperCase();
  ...
  const canContinue =
    step === 0
      ...
      : step === 1
        ...
        : step === 2
          ? Number(price) > 0
          : resolvedCurrency.trim().length === 3;
    ...
    <div className="mx-auto flex min-h-dvh w-full max-w-md flex-col px-6 pt-[max(3.5rem,env(safe-area-inset-top))] pb-[max(2rem,env(safe-area-inset-bottom))]">
      ...
      >
        <AnimatePresence mode="wait" initial={false}>
          {step === 0 && (
            <motion.div key="brand" {...slide} className="flex flex-1 flex-col pt-16">
          ...
          {step === 1 && (
            <motion.div key="size" {...slide} className="flex flex-1 flex-col pt-16">
          ...
          {step === 2 && (
            <motion.div key="price" {...slide} className="flex flex-1 flex-col pt-16">
          ...
          {step === 3 && (
            <motion.div key="currency" {...slide} className="flex flex-1 flex-col pt-16">
              ...
              <div className="mt-8 grid grid-cols-4 gap-2.5">
                {COMMON_CURRENCIES.map((c) => (
                  <motion.button
                    ...
                    className={`h-11 rounded-[14px] text-[15px] font-semibold transition-colors ${
                      !customCurrency && currency === c
                        ? "bg-accent text-white"
                        : "bg-fill text-label"
                    }`}
                  >
              ...
              <span className="mt-5 mb-1.5 block px-1 text-[13px] font-semibold tracking-[0.04em] text-label-secondary uppercase">
                Custom code
              </span>
              <input
